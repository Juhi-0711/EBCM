...
data[2:7] <- normalize(as.matrix(data[, 2:7]))
...
p=ggplot(train,aes(y=train$avg_time_spent, x= train$total_time_spent, color=train$status, shape=preds, size=train$num_resources/100) )+ geom_point(position=position_jitter(h=0.15,w=0.15))
